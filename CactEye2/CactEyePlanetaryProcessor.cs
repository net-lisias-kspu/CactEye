    ...
    class CactEyePlanetaryProcessor : CactEyeProcessor
    {
        public override string DoScience(Vector3 TargetPosition, float scienceMultiplier, float FOV, Texture2D Screenshot)
        {
            ...
            if (Target == FlightGlobals.Bodies[0])
            {
                ...
                {
                    Debug.Log("CactEye 2: Planetary Processor: Must be in orbit of a planet or moon");
                }
                return Type + ": Must be in orbit of planet or moon";
            }
            else if (TargetPosition == new Vector3(-1, -1, 0))
            {
                ...
                if (CactEyeConfig.DebugMode)
                {
                    Debug.Log("CactEye 2: Planetary Camera: Target not in scope.");
                }
